     ...
     l=$(grep -n "M	=	" ${liste[$i]} | cut -d : -f 1) # Extract the line number to replace
           for m in $(seq $Mmax)
               ...
               echo "INFO: compilation starts for the tool ${liste[$i]} at order m=$m"
                          ...
                          faust2supercollider -d -double -t 0 $output # Compilation with -d(ebug) flag : keep the .cpp and output files
                          rm $output $output2
                          rm *.scx *.sc *.so # Remove all generated files in the current dir
                          tmp=$(ls | grep "faust") # Temp dir generated by the compilation
