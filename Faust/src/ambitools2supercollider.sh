     ...
     l=$(grep -n "M	=	" ${liste[$i]} | cut -d : -f 1) # Extract the line number to replace
           for m in $(seq $Mmax)
               ...
               echo "INFO: compilation starts for the tool ${liste[$i]} at order m=$m"
                           ...
                           sed 's@0, *360@-1*ma.PI/2, ma.PI/2@g' $output2 > $output # Replace azimuth 0, 360 to -PI/2 PI/2
                           sed 's@-90, *90@-1*ma.PI/2, ma.PI/2@g' $output > $output2 # Replace azimuth 0, 360 to -PI/2 PI/2
                           mv  $output2 $output
