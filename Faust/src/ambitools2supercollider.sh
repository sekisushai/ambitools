    ...
    name=$suggest;
     ...
     for m in $sequence
               ...
               echo "INFO: compilation starts for the tool ${liste[$i]} at order m=$m"
                           ...
                           sed 's@0 *360@-1*ma.PI, ma.PI@g' $output2 > $output # Replace azimuth 0, 360 to -PI PI for osc message
                           sed 's@-90 *90@-1*ma.PI/2, ma.PI/2@g' $output > $output2 # Replace azimuth -90 90 to -PI/2 PI/2 for osc message
                           mv  $output2 $output
